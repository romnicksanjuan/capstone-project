...
const BarChartMerchandise = () => {
    ...
    useEffect(() => {
        ...
        const bargraph = async () => {
            try {
                const response = await fetch(`${DOMAIN}/bar-charts/merchandise`)

                const data = await response.json()
                // const d = data
                console.log("merchandise chart",data)
    ...
    return (
        <div style={{ width: "600px", height: "260px", margin: "0",borderRadius:"5px", backgroundColor:'whitesmoke', padding: '10px',  }}>
            <Bar data={data} options={options} style={{ height: "10px",}}/>
        </div>
    );
};

export default BarChartMerchandise;
